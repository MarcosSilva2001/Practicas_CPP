...
int main(int argc, char const *argv[])
{
    ...
    while (aux!=NULL)
    {
        fwrite(&aux->info,sizeof(int),1,Alum);
        aux=aux->sgte;
    }
    fclose(Alum);
...
void insertarOrdenado(Nodo*&Lista,Alumno valor)
{
   ...
   if(ant==NULL){//No entro al while 1)lista vacia 2)debo insertar al principio
      Lista=nuevo;
   }else{//Entro al while, 1) en alguna parte de la lista 2)agregar al final
      ant->sgte=nuevo;
   }
   nuevo->sgte=aux;
}
